...
{% materialization incremental, adapter='snowflake' -%}
  ...
  {% if on_schema_change == 'fail' and adapter.target_contains_schema_change( old_relation = old_relation, to_relation = old_relation) and not full_refresh_mode %}
    {{ exceptions.raise_fail_on_schema_change() }}
  {% elif on_schema_change == 'full_refresh' and adapter.target_contains_schema_change(old_relation = old_relation, to_relation = old_relation) and not full_refresh_mode  %}
