...
{% materialization incremental, adapter='snowflake' -%}
  ...
  {%- set tmp_relation = adapter.make_temp_relation(target_relation) -%}
  ...
  {% if on_schema_change == 'fail' and adapter.target_contains_schema_change( old_relation = tmp_relation, to_relation = target_relation) and not full_refresh_mode %}
    {{ exceptions.raise_fail_on_schema_change() }}
  {% elif on_schema_change == 'full_refresh' and adapter.target_contains_schema_change(old_relation = tmp_relation, to_relation = target_relation) and not full_refresh_mode  %}
